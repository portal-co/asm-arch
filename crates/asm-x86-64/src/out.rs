...
pub trait WriterCore {
    ...
    ///
    /// Exchanges the values in `dest` and `src`. If `mem` is provided,
    /// it specifies an offset for memory operand formatting.
    fn xchg(
        &mut self,
        cfg: crate::X64Arch,
        dest: &(dyn MemArg + '_),
    ...
    ///
    /// Copies the value from `src` to `dest`. If `mem` is provided,
    /// it specifies an offset for memory operand formatting.
    fn mov(
        &mut self,
        cfg: crate::X64Arch,
